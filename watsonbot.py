...
MAXIMUM_USER_PROFILE_STEP = 8
WATSON_CONFIDENCE_THRESHOLD = 20

def main():
    # Telegram Bot Authorization Token (@IamsterdamBot (IamsterdamBot))
    #bot = telegram.Bot('178800175:AAF0skUmAYjSI60CezycyVcrm9QKSJFz7Bk')
    
    # Telegram Bot Authorization Token (@DamskoBot (LocalBuddy))
...
def echo(bot, update_id, db, cur):
    ...
    for update in bot.getUpdates(offset=update_id, timeout=10):
        ...
        #removes emoticons (Watson doesn't like them as input)
        message = message.encode('ascii', 'ignore').decode('ascii')
		
        keyboard = None
        responseMessage = ""
        
        #if the message is not empty, generate the response
        if message:
        	keyboard, responseMessage = generateResponse(cur, db,  user_id, message, update.message.chat)
        
        #the user sent us only emoticons (which we have removed)
        else:
        ...
        if responseMessage:
            # Reply to the message if we have a response
...
def generateResponse(cur, db, user_id, message, update):
	if checkId(cur, user_id):
  		...
  		advertisement_step = getAdvertisementStep(cur, user_id)
  		
  		if userProfileStep < MAXIMUM_USER_PROFILE_STEP:
   			keyboard, response = introductionConversation(cur, db, user_id, message, userProfileStep)
  		else:
   			if advertisement_step > 0:
   				keyboard, response = advertisementConversation(cur, db, user_id, message, advertisement_step)
   			else:
...
def introductionConversation(cur, db, user_id, message, userProfileStep):
	...
	if userProfileStep == 0:
		...
		if message == "Yes":
			...
			response = "Don't worry, I can still answer your questions about Amsterdam! I can't give you personalized suggestions however. (Tip: type /deleteme to reset your account and preferences)"
			keyboard = keyboardmake([["What is a fun thing to do in Amsterdam?"],[getRandomQuestion()],["I have another question about Amsterdam!"]])
		...
		if message == "Yes" or message == "yes":
			...
			response = "Considering art, do you prefer modern or classic?"
		...
		if message == "modern":
			saveArtPreference(cur, db, user_id, "modern")
			response = "Finally, how many days are you staying?"
			incrementCurrentStep(cur, db, user_id)
			return keyboard, response
		
		elif message == "classic":
			saveArtPreference(cur, db, user_id, "classic")
			response = "Finally, how many days are you staying?"
			incrementCurrentStep(cur, db, user_id)
			return keyboard, response
		else:
			response = "Art: modern or classic?"
			keyboard = keyboardmake([["modern", "classic"]])
			return keyboard, response	

	elif userProfileStep == 7:
		try:
			message = int(message)
			saveDaysStaying(cur, db, user_id, message)
			incrementCurrentStep(cur, db, user_id)
			keyboard = keyboardmake([["What is a fun thing to do in Amsterdam?"],[getRandomQuestion()],["I have another question about Amsterdam!"]])
			response = "Awesome, how can I help? (Tip: type /me to check your profile)"
			return keyboard, response
		except ValueError:
...
def regularConversation(cur, db, user_id, message):
	...
	if message == "What is a fun thing to do in Amsterdam?":
		if userHasProfile(cur, user_id) and userHasPreference(cur, user_id, "museum_preference"):
			response = generateProfiledAttraction(cur, user_id)
		else:
			response = generateRandomAttraction(cur, user_id)
		...
		response = "Your account has been reset. How can I help?"
		keyboard = None
	
	elif message == "/me":
		response = generateAccount(cur, user_id)
		keyboard = None
	
	elif message == "/advertisement":
		response = getFirstName(cur, user_id)+ ", how are you? I've heard something new!"
		keyboard = None
		setAdvertisementStep(cur, db, user_id, 1)
	
	else:
		answerTitle, answerText, answerConfidence = getAnswerDetails(message, 0)
		
		if answerTitle != "" and answerConfidence > WATSON_CONFIDENCE_THRESHOLD:
...
#example advertisement conversation
def advertisementConversation(cur, db, user_id, message, advertisement_step):
	if advertisement_step == 1:
		response = "Would you like to go to a party tonight??"
		keyboard = keyboardmake([["YES", "nope"]])
		setAdvertisementStep(cur, db, user_id, 2)
		return keyboard, response
	
	elif advertisement_step == 2:
		if message == "YES":
			response = "Great! Do you like awesome clubs?"
			keyboard = keyboardmake([["YES", "nope"]])
			setAdvertisementStep(cur, db, user_id, 3)
			return keyboard, response
		elif message == "nope":
			response = "Too bad."
			keyboard = keyboardmake([["What is a fun thing to do in Amsterdam?"],[getRandomQuestion()],["I have another question about Amsterdam!"]])
			setAdvertisementStep(cur, db, user_id, 0)
			return keyboard, response
		else:
	...
	elif advertisement_step == 3:
		if message == "YES":
			response = "COOL! Then come to Paradiso tonight! Find more information and buy tickets at www.paradiso.nl. Here is your coupon for 5% discount: [789-ads]"
			keyboard = keyboardmake([["What is a fun thing to do in Amsterdam?"],[getRandomQuestion()],["I have another question about Amsterdam!"]])
			setAdvertisementStep(cur, db, user_id, 0)  
			return keyboard, response
		elif message == "nope":
...
#return title, text and confidence of the selected Watson answer for a given question	
def getAnswerDetails(question, position_to_top):
	answerTitle = ""
	answerText = ""
	answerConfidence = 0
	...
	if 'question' in r.json():
		if 'value' in r.json()['question']['evidencelist'][position_to_top]:
			answerTitle = r.json()['question']['evidencelist'][position_to_top]['title']
			answerText = r.json()['question']['evidencelist'][position_to_top]['text']
			answerConfidence = r.json()['question']['evidencelist'][position_to_top]['value']
...
#returns a string with a random question that is suited for Watson   
...
#returns true if input string is equal to one of our age categories
...
#returns true if the user has a profile (check if gender is not empty and introduction is completed)
def userHasProfile(cur, user_id):
	gender = getGender(cur, user_id)
	user_step = returnCurrentStep(cur, user_id)
	
	if gender != None and user_step == MAXIMUM_USER_PROFILE_STEP:
		return True
	else:
		return False

#returns a string with the user account information
def generateAccount(cur, user_id):
	first_name = getFirstName(cur, user_id)
	ageCategory = getAge(cur, user_id)
	gender = getGender(cur, user_id)
	country = getCountry(cur, user_id)
	days_staying = getDaysStaying(cur, user_id)
	date = getCreationDate(cur, user_id)
	museum_preference = str(getPreference(cur, user_id, "museum_preference"))
	
	account = "Hello there " + first_name + "!"
	
	if userHasProfile(cur, user_id):
		account = account + " I believe you are a " +gender+ " aged " +ageCategory+ "." 
		account = account + " I think you are from " + country + ", but that is just a wild guess haha."
		account = account + " You are visiting Amsterdam for " +str(days_staying)+"  days and this account was created on "+str(date)+"."
		
		if museum_preference == "1":
			account = account + " During your stay you are willing to visit museums."
			art_preference = getPreference(cur, user_id, "art_preference")
			account = account + " I believe you are into " + art_preference + " art."
		else:
			account = account + " You are not interested in visiting museums during your stay."
		
		account = account + " Type /deleteme if you are not satisfied with your account and preferences."
		
	else:
		account = account + " I don't know a whole lot about you since you didn't answer my introductory questions. Type /deleteme to reset your account and preferences."
	
	return account

#returns a string with the title, short description and url of a random Amsterdam attraction that does not violate user preferences
def generateRandomAttraction(cur, user_id):
	...
	if websites != "":
		attraction = attraction + " For more info visit: " + websites
	
	return attraction

def generateProfiledAttraction(cur, user_id):
	attraction_type = "museum" #we would want to pick an attraction type at random that does not conflict with user profile and is suited for the time of the day
	topic = "art" #within this attraction type, we would want to select a topic that we've asked the user a question about
	user_art_preference = getPreference(cur, user_id, "art_preference") #we select the preference of the user for this selected topic from our database
	
	watson_question = attraction_type +" "+ user_art_preference+" "+topic
	
	title, text, confidence = getAnswerDetails(watson_question, 0)
	
	return title

#returns false when the user preference does not match the (structured) attraction type	
...
def getAge(cur, userId):
	query = "select age from users where id = " + str(userId)
	cur.execute(query)
	age = cur.fetchone()[0]
	return age
	
def getGender(cur, userId):
	query = "select gender from users where id = " + str(userId)
	cur.execute(query)
	gender = cur.fetchone()[0]
	return gender

def getCountry(cur, userId):
	query = "select country from users where id = " + str(userId)
	cur.execute(query)
	country = cur.fetchone()[0]
	return country

def getPreference(cur, userId, preference):
	query = "select " + preference + " from users where id = " + str(userId)
	cur.execute(query)
	preference = cur.fetchone()[0]
	return preference

def getDaysStaying(cur, userId):
	query = "select stay_days from users where id = " + str(userId)
	cur.execute(query)
	days = cur.fetchone()[0]
	return days

def getCreationDate(cur, userId):
	query = "select date_created from users where id = " + str(userId)
	cur.execute(query)
	date = cur.fetchone()[0]
	return date
...
def setLastCurrentStep(cur, db, userId):
    query = "update users set current_step = "+ str(MAXIMUM_USER_PROFILE_STEP) +" where id =" + str(userId)
    cur.execute(query)
    db.commit()
    pass

def getAttractionTitle(cur, attractionId):
	query = "select TitleEN from attractions where Trcid ='" + str(attractionId)+"'"
	cur.execute(query)
...
def getAdvertisementStep(cur, userId):
	query = "select advertisement_step from users where id = " + str(userId)
	cur.execute(query)
	step = cur.fetchone()[0]
	return step

def setAdvertisementStep(cur, db, userId, step):
	query = "update users set advertisement_step = "+str(step)+" where id =" + str(userId)
	cur.execute(query)
	db.commit()
	pass
